...
// FE Power Monitoring stmpbuf[27], stmpbuf[28] -- Check voltage and dB conversio?4/4/2014 Parikh //
	   ...
	   vn2 = ((float)stmpbuf[26]*10)/(float)256;
...
//--------FE
    if (  (int) stmpbuf[pt] < 100 && (int) stmpbuf[pt+2] > 100) Monparam[i*3] = 150;
           ...
           if(i==0){ strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"BAND_SELECT_M_CH2"); }
           else {strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"BAND_SELECT_M_CH1"); }
       sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%d",Monparam[i*3]);
       resp_msg_cntr++;
 ...
 //--------SOLATTEN
    ...
    if(i==0){ strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"SOL_ATTEN_M_CH2");}
    else  { strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"SOL_ATTEN_M_CH1");}
    sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%d",Monparam[1+(i*3)]);
    resp_msg_cntr++;

 //----------CB_DET
    if(i==0){ strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"CB_DET_CH2");}
    else {strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"CB_DET_CH1");}
     
     // fprintf(stderr,"###################################Value in STMPBUF[%x]\n",stmpbuf[pt+5]);
      sprintf(t,"%x",stmpbuf[pt+5]);
    //  printf("##############%s\n",t);
       l = strtol(t,NULL, 16);
       fl = (float)l;
     // fprintf(stderr,"###################################Value in FLOAT[%f]\n",fl);
     pow_db=(double)convertdB(fl, 1); 
     //pow_db=(double)convertdB((float)atof((char *)stmpbuf[pt+5]), 1); 
     sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%g",pow_db );
    //sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%4.2f",1.0 );
    resp_msg_cntr++;
...
//------------------ VCC_M
          sprintf(t,"%x",stmpbuf[pt+2]);
         // printf("##############%s\n",t);
          l = strtol(t,NULL, 16);
          fl = (float)l;
         // fprintf(stderr,"###################################Value in FLOAT[%f]\n",fl);
          Monvoltbis[i*3] = (float)(fl*10)/(float)256 -5;
        /* Monvoltbis[i*3] = ((float)atof((char *)stmpbuf[pt+2])*10)/(float)256 -5; */
        if(i==0){strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"VCC_M_CH2");}
        else{strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"VCC_M_CH1");}
        sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%4.2f",Monvoltbis[i*3]);
        resp_msg_cntr++;

//------------------ VEE_M
          sprintf(t,"%x",stmpbuf[pt+3]);
         // printf("##############%s\n",t);
          l = strtol(t,NULL, 16);
          fl = (float)l;
         // fprintf(stderr,"###################################Value in FLOAT[%f]\n",fl);
          Monvoltbis[1+i*3] = (float)(fl*10)/(float)256 -5;
        /* Monvoltbis[1+i*3] = ((float)atof((char *)stmpbuf[pt+3])*10)/(float)256 -5; */
        if(i==0){strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"VEE_M_CH2");}
        else {strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"VEE_M_CH1");}
        sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%4.2f",Monvoltbis[i*3]);
        resp_msg_cntr++;
 ...
 //------------------ VREF_M
         ...
         //  printf("##############%s\n",t);
           ...
           fl = (float)l;
        //  fprintf(stderr,"###################################Value in FLOAT[%f]\n",fl);
          Monvoltbis[2+i*3] = (float)(fl*10)/(float)256 -5;
       // Monvoltbis[2+i*3] = ((float)atof((char *)stmpbuf[pt+4])*10)/(float)256 -5;
        if(i==0){strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"VREF_M_CH2");}
        else {strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"VREF_M_CH1");}
        sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%4.2f",Monvoltbis[i*3]);
        resp_msg_cntr++;

  //------------------ RF_POW_M
         if(i==0){strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"RF_POW_M_CH2");}
         else  {strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"RF_POW_M_CH1");}
    ...
    //   printf("##############%s\n",t);
       ...
       fl = (float)l;
    //   fprintf(stderr,"###################################Value in FLOAT[%f]\n",fl); 
       sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%4.2f",fl);
      //  sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%4.2f",atof((char *)stmpbuf[pt+5]) );
        resp_msg_cntr++;

 // ---------------- CB_TEMP
     if (i == 0 ) {
         strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"CB_TEMP");
         sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%4.2f",  (( (float)stmpbuf[52] * 39.0625) - 5000.0 )/80.0 );
          resp_msg_cntr++;
     }
...
void decodeResponse()
{
  ...
  char *cmdName[] = { "setMCMAddr", "NullCmd", "SetIdleMode", "SetScanMode", "SetMeanMode", "setAnalogMask", "setDigMask16Bits", "setDigMask32Bits",\
   ...
   /*  for(j = 0; j <rbuff.Length ; j++) 
         ...
         //  printf("++++++++++ START %d %d %d \n", resp_msg_cntr,rawDataLen,remRespLen);


        while( remRespLen > 0 )
        {
             // printf("++++++++++ INSIDE WHILE LOOP START %d %d %d \n", resp_msg_cntr,rawDataLen,remRespLen); 
               RxCmdNum = (-1);

		if((int)rbuff.Msg[cnt] == 0)
		{ 
                   printf("mcmnb = %d, cnt = %d \n", (int)rbuff.Msg[cnt], cnt);
                    mcmnb = (int)rbuff.Msg[cnt++];
                   printf("No Of Response %d, MCMCard %d\n",  nCmdResps, mcmnb);
                }
                 else if((int)rbuff.Msg[cnt] == 10)
                { 
                   printf("mcmnb = %d, cnt = %d \n", (int)rbuff.Msg[cnt], cnt);
                    mcmnb = (int)rbuff.Msg[cnt++];
                  printf("No Of Response %d, MCMCard %d\n",  nCmdResps, mcmnb);
                }
               else if((int)rbuff.Msg[cnt] == 2)
                { 
                   printf("mcmnb = %d, cnt = %d \n", (int)rbuff.Msg[cnt], cnt);
                    mcmnb = (int)rbuff.Msg[cnt++];
                  printf("No Of Response %d, MCMCard %d\n",  nCmdResps, mcmnb);
                }
                  else if((int)rbuff.Msg[cnt]==3)
                { 
                   printf("mcmnb = %d, cnt = %d \n", (int)rbuff.Msg[cnt], cnt);
                    mcmnb = (int)rbuff.Msg[cnt++];
                  printf("No Of Response %d, MCMCard %d\n",  nCmdResps, mcmnb);
                }
                 else {cnt++; remRespLen--; continue; }
