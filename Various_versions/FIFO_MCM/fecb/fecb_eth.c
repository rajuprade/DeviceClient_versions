...
int command_process_FE(parseCMSCmd *Wrpr_Cmd, devResponse *MCM_Resp)
{
  int arg_fpscnt=0;
  int i, j ;
  char *fevar[] =  { "band_select_ch1" ,"band_select_ch2" ,"rf_swap" ,"sol_atten_ch1" ,"sol_atten_ch2" ,"fe_ngcal" ,"fe_walsh_sw" ,"fe_walsh_grp" ,"fe_ngcycle" ,"rfcm_sw", "ngset","fe_ng_freq","fe_walsh_pat","fe_walsh_freq"};
  typedef enum { BAND_SELECT_CH1, BAND_SELECT_CH2, RF_SWAP, SOL_ATTEN_CH1, SOL_ATTEN_CH2, FE_NGCAL, FE_WALSH_SW, FE_WALSH_GRP, FE_NG_CYCLE, RFCM_SW, NGSET,FE_NG_FREQ,  FE_WALSH_PAT, FE_WALSH_FREQ, TSET};
...
//  printf("\nEntering Command_Process_FE");
  ...
  else if(! strcmp(Wrpr_Cmd->cmdelem.cmd_name,Cmd_202))
  {
    ...
    fecbSet.cmdCode = 2;
    printf("In Command Process FE => SET RF SYS =>\n");
  }
  ...
  { for (i = 0 ; i < TSET; i++ ) {
     ...
     { 
        ...
        switch(i)
        {
             ...
             case FE_NG_FREQ     :  fecbSet.ngFreq =(int) atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);
                                    if( fecbSet.ngFreq > 3 || fecbSet.ngFreq  < 0 )
                                    fecbSet.ngFreq = 0;
                                     break;

             case FE_WALSH_PAT   :  fecbSet.walshPat =(int)atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);
                                    if( fecbSet.walshPat > 7 || fecbSet.walshPat  < 0 )
                                     fecbSet.walshPat = 0;
                                     break;

             case FE_WALSH_FREQ  :  fecbSet.walshFreq =(int)atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);
