 ...
 * program modified so that post gain can be given in step size of .5 DB // Date of Modification 14/05/2014

 * Post gain setting has been successfully tested on 15/05/2014.
 * In the program sequence of ALC & Post Gain has been ultered & Post gain setting started working.
 * SET_IF command has been tested throughly. => Raj/Hari/Abhijeet/Sudhir => 15/05/2014 
...
#include <pthread.h>
#include<fcntl.h>
#include "/data/LinuxMCM/Hello/ftd2xx.h"
...
// sTime = time(NULL);
	/* Setup ANALAB USB DEVICE ID "FTUN7KZE"*/
         
            if((ftStatus = FT_OpenEx("FTUN7KZE", FT_OPEN_BY_SERIAL_NUMBER, &ftHandle)) != FT_OK)
            //  if((ftStatus = FT_OpenEx("FTVX10DN", FT_OPEN_BY_SERIAL_NUMBER, &ftHandle)) != FT_OK)
             //if((ftStatus = FT_OpenEx("FTU6Z938", FT_OPEN_BY_SERIAL_NUMBER, &ftHandle)) != FT_OK)
      //     if((ftStatus = FT_OpenEx("FTUN7QVO", FT_OPEN_BY_SERIAL_NUMBER, &ftHandle)) != FT_OK)
           //if((ftStatus = FT_OpenEx("FTUN7NUJ", FT_OPEN_BY_SERIAL_NUMBER, &ftHandle)) != FT_OK)
          // if((ftStatus = FT_OpenEx("FTUN7Q3F", FT_OPEN_BY_SERIAL_NUMBER, &ftHandle)) != FT_OK)
...
Mask64toArg(char *buf_ptr0,char mask0[8], char mask1[8], char mask2[8], char mask3[8])
{
  ...
  printf("#################################################################\n");
    ...
    buff[15] = Arg[6];
    buff[16] = Arg[7];
 printf("MASKTOARG64 => HEX Argument in Buff %x %x %x %x %x %x %x %x\n",buff[9],buff[10],buff[11],buff[12],buff[13],buff[14],buff[15],buff[16]);
...
void SET_IF(char *buf_ptr0 )
{
    ...
    int band_width[]= {   6,   16,   32, -1};
      ...
      fprintf(stderr,"select pre_attenuator for CH1 & CH2 (0,2,4,...,30 dB).)\n");
      scanf("%d %d",&set_para[0],&set_para[2]);
     fprintf(stderr,"select POST GAIN for CH1 & CH2.(In the step size of 0.5 DB).)\n");
      scanf("%f %f",&post_gain[0],&post_gain[1]); 
...
buff[15] = Argument[7];
buff[16] = Argument[6];
printf("SET DMASK 64 bit=> %x %x %x %x %x %x %x %x\n",buff[9],buff[10],buff[11],buff[12],buff[13],buff[14],buff[15],buff[16]);
