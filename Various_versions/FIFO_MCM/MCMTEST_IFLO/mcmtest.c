...
void write_read(char *buff)
{
   ...
   length = buff[1] + (buff[2]<<8);
    ...
    respiflo=RESP_IFLO;
    resp_fd = open(respiflo,O_WRONLY|O_NONBLOCK); 
    
             if(resp_fd==-1)
               { printf("Problem in Opening RESPONSE IFLO FIFO\n");printf("Unable to OPEN a RESPONSE IFLO FIFO ; errno=%d\n",errno); }
              else
               { printf("Successful in Opening RESPONSE IFLO FIFO\n");
               } 
...
{ 
    ...
    { printf("Successful in Opening IFLO FIFO\n");} 
