...
{
   int i,length,ret=0;
   resp_iflofd = open(respiflo,O_RDONLY|O_NONBLOCK); 
       if(resp_iflofd == -1)
       {
    ...
    add_cksum(buff);  
     ...
     printf("RESPONSE IFLO FIFO ID in Command_process() ====> %d\n",resp_iflofd);
    // usleep(2000);
    write(iflo_fd, buff, (length+1)); 
    printf("========================================================\n");   
    for (i=0; i<(length+1); i++)
    printf("0x%02X\t",(unsigned char)buff[i]);
    printf("\n========================================================\n");
    sleep(1);
    ret=read(resp_iflofd,rbuff.Msg,100);
