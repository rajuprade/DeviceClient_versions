...
void main(int argc, char *argv[])
{
     ...
     /* resp_fd = open(respfps,O_RDONLY|O_NONBLOCK); 
      if(resp_fd == -1)
      {
         printf("Problem in Opening RESPONSE FPS FIFO \n"); printf("Unable to create RESPONSE FIFO fifo; errno=%d\n",errno);
      }
      else
      {
       printf("RESPONSE FPS FIFO ID in main() ====> %d\n",resp_fd);
      }*/
    extern int reqFd ;
    parseCMSCmd *cmdptr;
    devResponse *respptr;
  ...
  while(1) 
  {
            ...
            else
           {
             printf("RESPONSE FPS FIFO ID in main() ====> %d\n",resp_fd);
              ...
              FPS_form_inter_resp(cmdptr,respptr);
              sendRespPkt(&reqFd,respptr);
              FPS_form_final_resp(cmdptr,respptr);
                 respptr= &dev_Resp;
               sendRespPkt(&reqFd,respptr);// printDevResp(respptr);
              //formDeviceResp(cmdptr, respptr);
               close(fps_fd);
             //  close(resp_fd);
           }
