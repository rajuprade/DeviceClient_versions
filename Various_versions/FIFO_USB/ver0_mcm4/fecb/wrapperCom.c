...
int allocComBuf() 
{
   ...
   if ( (cmdBuf = (unsigned char *) calloc(parseCMSCmdSize, 1)) == (unsigned char *)NULL ) 
    { fprintf(stderr, " allocComBuf : could not allocate memory\n");
      return ERROR ;
    }

   if ( (respBuf = (unsigned char *) calloc(devResponseSize, 1) ) == (unsigned char *)NULL ) 
    { fprintf(stderr, " allocComBuf : could not allocate memory\n");
      return ERROR ;
    }
    return 0;
...
#define _GNU_SOURCE
 ...
 // recvpoll.events= POLLHUP|POLLRDHUP|POLLNVAL ; // no need to set the flag?
    ...
    switch((short)recvpoll.revents)  {
         ...
         // Error Condition
          case POLLHUP : fprintf(stderr, " ### Error recvRespPkt() : poll - Signal occurred  POLLHUP  %d\n",(short)recvpoll.revents);
                          return (int) recvpoll.revents; 
/*
          case POLLRDHUP : fprintf(stderr, " ### Error recvRespPkt() : poll - Signal occurred  POLLRDHUP  %d\n",(short)recvpoll.revents);
                          return (int) recvpoll.revents; 
*/
          case POLLNVAL :  fprintf(stderr, " ### Error recvRespPkt() : poll - Signal occurred  POLLNVAL  %d\n",(short)recvpoll.revents);
                          return (int) recvpoll.revents; 
      }
      return 0 ;
 }
...
//      fprintf(stderr, "### Wanting for Command: recvCmdPkt() [%d] !!\n",(int)bytes);
      return -1;
  } else {
    fprintf(stderr, "recvCmdPkt() Number of Bytes [%d] \n",(int) bytes);
    //printCmdPkt(cmdPKT);
    return parseCMSCmdSize ;
  }     
...
int initClientConn(int clientPort)
{
 ...
 reuse=1;
 if(setsockopt(sock, IPPROTO_TCP,TCP_NODELAY, (char *)&reuse,sizeof(reuse)) == ERROR)
