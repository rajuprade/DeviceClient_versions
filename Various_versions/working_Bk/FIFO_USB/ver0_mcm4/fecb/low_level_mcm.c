...
{
   int i,length,ret=0;
   resp_fecbfd = open(respfecb,O_RDONLY|O_NONBLOCK); 
              if(resp_fecbfd == -1)
             {
               printf("Problem in Opening RESPONSE FECB FIFO \n"); printf("Unable to create RESPONSE  FECBFIFO fifo; errno=%d\n",errno);
             }
            else
           {
             printf("RESPONSE FECB FIFO ID in main() ====> %d\n",resp_fecbfd);
           }
    buff[0]=*buf_ptr0; 
    ...
    add_cksum(buff);  
     ...
     printf("RESPONSE FECB FIFO ID in Command_process() ====> %d\n",resp_fecbfd);
    // usleep(2000);
    write(fifo_fd, buff, (length+1));    
   for (i=0; i<(length+1); i++)
    printf("serial PKT in LOW_Level ====>0x%02X\n",(unsigned char)buff[i]);
   sleep(1);
    ret=read(resp_fecbfd,rbuff.Msg,100);
...
void set_anl_mask()
{
 ...
 buff[17]=0;
  ...
  for(i=0; i<8; i++)
  buff[9+i] = exeCmd.Argument[i]; //write_read(buff,buf_ptr0,rbuff);  
 // InitBuff(buff);
}
...
void set_mean_mode()
{
 ...
 buff[10]=0;
 buff[11]=0;
 buff[10] = exeCmd.Argument[0]; //write_read(buff,buf_ptr0,rbuff);  
 //InitBuff(buff);
}
...
void feed_cntrl_new()
{
  ...
  buff[10]=0;
  for(i=0; i<2; i++)
  buff[8+i] = exeCmd.Argument[i]; //write_read(buff,buf_ptr0,rbuff);  
 // InitBuff(buff);
}
...
void FECB_CTRL(int MCMADDR )
{
  ...
  switch(exeCmd.CmdNum)
  {
       ...
       case 3 : // Scan Mode
                 ...
                 buff[8]=0;
                 buff[9]=1;
                 buff[10]=0;// write_read(buff,buf_ptr0,rbuff);  
                // InitBuff(buff);
                 break;
       case 4 : // Mean Mode -- Unused.
                 set_mean_mode(); //write_read(buff,buf_ptr0,rbuff);  
                 break;
       ...
       case 9 : // TX Read Anl Mask
                //buff[]={0,9,0,0,2,0,1,0,0,0};
                 ...
                 buff[7]=0;
                 buff[8]=0;
                 buff[9]=0; //write_read(buff,buf_ptr0,rbuff);  
                // InitBuff(buff);
                break;
      ...
      case 15 : // TX Reboot
                 ...
                 buff[6]=0;
                 buff[7]=0;
                 buff[8]=0;// write_read(buff,buf_ptr0,rbuff);  
                 //InitBuff(buff);
                 break;
      case 16 : // TX Feed Mcm
