...
static tcp_Socket OFC_Socket, FPS_Socket;
...
cofunc int FPS_mcm_clnt()
{
   int port, write_bytes, read_bytes, wait_var;
   longword host;

   if(!(host = inet_addr(FPS_SERVER_ADDR)))
   {
      puts("\nCould not resolve host");
      exit(3);
   }
   ...
   port = atoi(FPS_SERVER_PORT);

   printf("\nAttempting to open '%s' on port %u\n", FPS_SERVER_ADDR, port );

   while( ! tcp_open(&FPS_Socket, 0, host, port, NULL))
    {
     tcp_tick(NULL);
     printf("\nUnable to open TCP session");
    }
   ...
   while((!sock_established(&FPS_Socket) && sock_bytesready(&FPS_Socket)== -1))
    {
      http_handler();
      tcp_tick(NULL);
      wait_var++;
      if(wait_var == 100)
      {
        tcp_open(&FPS_Socket, 0, host, port, NULL);
        wait_var = 0;
      }
      yield;
    }
   printf("\nFPS Socket Established ****************************************\n");

   if(sock_established(&FPS_Socket))
   {
    read_bytes = 0;
    do{
       http_handler();
       if(sock_bytesready(&FPS_Socket) > 0)
       {
         read_bytes = sock_read(&FPS_Socket, &FPS_Wrpr_Cmd, Wrpr_Cmd_Size);

         printf("\nMCM has read command of %d bytes",read_bytes);

         if(FPS_form_inter_resp() == 10)    // Form intermediate response and send it over socket //
         {
           write_bytes = sock_write(&FPS_Socket, &FPS_MCM_Resp, MCM_Resp_Size);
           printf("\nMCM has sent intermediate response of %d bytes",write_bytes);
           printf("\nIntermediate response :\n %d \n",FPS_MCM_Resp.event);
           http_handler();
           FPS_form_final_resp(SUCCESS,FINAL);   // Process the command, Form final response and send it over socket //
           write_bytes = sock_write(&FPS_Socket, &FPS_MCM_Resp, MCM_Resp_Size);
       ...
       read_bytes = 0;
       yield;
      }while(tcp_tick(&FPS_Socket));    // This loop continuously listens over socket for incoming command //
   }
   http_handler();
...
main()
{
  ...
  while(1)
  {
    ...
    costate    // This will open client socket at port 3004 //
     {
       wfd FPS_mcm_clnt();
     }
...
int FPS_cmd_validation()
 {
   ...
   if(!(_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.subsysid,SYSTEM_4)))
       {
         printf("\nSubsystem validated");
         _f_strcpy(cmdstr2,FPS_Wrpr_Cmd.cmd_elem.cmd_name);
         fprintf(stdout,"\nCommand received : %s\n",  cmdstr2);

         if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_401))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_402))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_403))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_404))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_405))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_406))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_407))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_408))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_409))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_410))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_411))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_412))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_413))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_414))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_415))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_416))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_417))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_418))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_419))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_420))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_421))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_422))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_423))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_424))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_425))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_426))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_427))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_428))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_429))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_430))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_431))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_432))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_433))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_434))
          {
           return 10;
          }
         else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_435))
          {
           return 10;
          }
           ...
           for(j=0; j<FPS_Wrpr_Cmd.cmd_data.numpkt; j++) // Looking for Jth parameter //
            {
             for(i=0; i<SMP_rawsize; i++) // comparing Jth against Ith parameter //
              {
               if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_data.prmname[j],SenMon_Para[i]))
                {
                 cmpl2++;
                 break;
                }
            ...
            if(cmpl2 == FPS_Wrpr_Cmd.cmd_data.numpkt)
             return 10;
            else
             return 13;   // Incase of Incomplete command //
          }
          else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_437))
          {
           return 10;
          }
         else
...
int FPS_form_inter_resp()
{
  ...
  FPS_MCM_Resp.code = FPS_cmd_validation();
  FPS_MCM_Resp.event = INTERMEDIATE;
  FPS_MCM_Resp.MCM_alarm.level=1;
  switch(FPS_MCM_Resp.code)
  {
    case 10 :  _f_strcpy(FPS_MCM_Resp.message,"Command received successfully");
               printf("\nCommand received successfully");
               break;
    case 12 :  _f_strcpy(FPS_MCM_Resp.message,"Command Syntax Error");
               printf("\nCommand Syntax Error");
               break;
    case 13 :  _f_strcpy(FPS_MCM_Resp.message,"Command Incomplete");
               printf("\nCommand Incomplete");
               break;
    case 14 :  _f_strcpy(FPS_MCM_Resp.message,"Command Invalid");
               printf("\nCommand Invalid");
               break;
    case 100:  _f_strcpy(FPS_MCM_Resp.message,"Error Unknown");
               printf("\nError Unknown");
               break;
    default :  break;
  };
 ...
 // ******** Write in to Tx_BasicFlds *********** //
  FPS_MCM_Resp.resp_elem.seq = FPS_Wrpr_Cmd.cmd_elem.seq;
  FPS_MCM_Resp.resp_elem.version = FPS_Wrpr_Cmd.cmd_elem.version;
  _f_strcpy(FPS_MCM_Resp.resp_elem.subsysid,FPS_Wrpr_Cmd.cmd_elem.subsysid);
  _f_strcpy(FPS_MCM_Resp.resp_elem.cmd_id,FPS_Wrpr_Cmd.cmd_elem.cmd_id);
  _f_strcpy(FPS_MCM_Resp.resp_elem.cmd_name,FPS_Wrpr_Cmd.cmd_elem.cmd_name);
  _f_strcpy(FPS_MCM_Resp.resp_elem.timestamp,FPS_Wrpr_Cmd.cmd_elem.timestamp);
  FPS_MCM_Resp.resp_elem.priority = FPS_Wrpr_Cmd.cmd_elem.priority;
  FPS_MCM_Resp.resp_elem.timeout = FPS_Wrpr_Cmd.cmd_elem.timeout;

  // ******** Write in to Tx_DataPkt *********** //
  FPS_MCM_Resp.resp_data.numpkt = 0;
  for(loop=0; loop<MAXDATA; loop++)
   {
     _f_strcpy(FPS_MCM_Resp.resp_data.prmname[loop],"\0");
     _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[loop],"\0");
   }
  // ******** Write in to Tx_alarm *********** //
  FPS_MCM_Resp.MCM_alarm.id = 0;
  FPS_MCM_Resp.MCM_alarm.level = 0;
  _f_strcpy(FPS_MCM_Resp.MCM_alarm.description,"\0");
...
void FPS_form_final_resp(int code, int event)
{
  //printf("\nEntering form_final_resp");
  FPS_resp_msg_cntr = 0 ;
  FPS_resp_code = 10;
  FPS_MCM_Resp.code = FPS_resp_code;
  FPS_MCM_Resp.event = command_process_fps();
...
int command_process_fps()
{
 ...
 // _f_strcpy(Wrpr_Cmd.cmd_elem.cmd_name, "domon");
  ...
  if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_401))        // null/domon
  {
  printf("\nfps command: null \n");
  out_data[0] = 8;
  out_data[1] = 0;
  ...
  arg_fpscnt = _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0])/2 ;
  ...
  out_data[8] = (int) _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0])/5 ;  // time count
  ...
  arg_fpscnt = (int) ((15 * 140625)/(160 * _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0]))) ;
  /*   teleset-abc
  ...
  */
  ...
  out_data[8] = (int) _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0])/2;   // Encoder Pulses/2
  out_data[9] = chksum(&out_data[0]);     // calculate checksum
  transmit();
  ser2eth_comm();
  }
  ...
  out_data[8] = (int) _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0])/5;  // TMcount/5
  out_data[9] = chksum(&out_data[0]);     // calculate checksum
  transmit();
  ser2eth_comm();
  }
  ...
  out_data[8] = (int) _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0])/ 5;   // timecount
  out_data[9] = chksum(&out_data[0]);     // calculate checksum
  transmit();
  ser2eth_comm();
  }
  ...
  arg_fpscnt = _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0])/2 ; // Encoder pulse
  out_data[0] = 0x0b;
  out_data[1] = 0;
  out_data[2] = 0;
  out_data[3] = 1;  // set = 1  // main cmd
  ...
  out_data[8] = _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0]);
  out_data[9] = chksum(&out_data[0]);     // calculate checksum
  transmit();
  ser2eth_comm();
  }
  ...
  out_data[8] = _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0]);      // 0=270 or 1=-10 direction of fps
  out_data[9] = chksum(&out_data[0]);     // calculate checksum
  transmit();
  ser2eth_comm();
  }
...
//  out_data[9] = _f_atoi(Wrpr_Cmd.cmd_data.prmvalue[1]);      // tgt angle
  ...
  arg_fpscnt = (_f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0]) * 28.44)+ (8500 - (28.44 * 270));
  // 270 is hardcoded.
  out_data[0] = 0x0b;
  out_data[1] = 0;
  out_data[2] = 0;
 ...
 // out_data[9] = _f_atoi(Wrpr_Cmd.cmd_data.prmvalue[1]);      // tgt angle
  ...
  out_data[8] = (int ) arg_fpscnt & 0x00ff ;
  out_data[9] = (int ) arg_fpscnt >> 8;
  out_data[10] = _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[2]);     //
  ...
  fps_def_cnt[0] = _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[0]); // 610
  fps_def_cnt[1] = _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[1]); // 150
  fps_def_cnt[2] = _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[2]); // 1420
  fps_def_cnt[3] = _f_atoi(FPS_Wrpr_Cmd.cmd_data.prmvalue[3]); // 325
  //transmit();
  FPS_resp_code = 10;
  //ser2eth_comm();
  }
...
//  out_data[9] = _f_atoi(Wrpr_Cmd.cmd_data.prmvalue[1]);      // tgt angle
  ...
  else if(!_f_strcmp(FPS_Wrpr_Cmd.cmd_elem.cmd_name,Cmd_438))   // setttime will set RTC //
  {
    Set_RTC(FPS_Wrpr_Cmd.cmd_elem.timestamp);
  }
  ...
  {
    printf("Nothing Matched, Received : %s",FPS_Wrpr_Cmd.cmd_elem.cmd_name);
  }
...
break;
   ...
   if(bytecntr_in <= 0 )
   {
   _f_strcpy(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr],"cmdstatus");
   _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"No Answer from FPS");
   printf("\nNo Answer from FPS");
   FPS_resp_msg_cntr++;
   FPS_resp_code = 11;
   return;
...
{//printf("pkt size error. \n");
 _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Packet Size error");
 printf("\nPacket Size error");
  FPS_resp_msg_cntr++;
  FPS_resp_code = 11;
 return ;
...
printf("NOT FPS 14 Packet.\n");
 sprintf(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr],"NOT FPS 14 Packet %d - %s" ,bytecntr_in,in_d);
 FPS_resp_msg_cntr++;
 FPS_resp_code = 11;
 return ;
...
// check status byte.
 ...
 if(in_d[i+4] == 0x00)
  _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Exec OK");
 if(in_d[i+4] & 0x01)
  _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"FPS detected timeout");
 if(in_d[i+4] & 0x02)
  _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"FPS detected CS error");
 if(in_d[i+4] & 0x04)
  _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"FPS received unknown command");
 if(in_d[i+4] & 0x08)
  _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"FPS received unknown command");
 if(in_d[i+4] & 0x10)
  _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"FPS received correct Pkt but rejected cmd");
 if(in_d[i+4] & 0x20)
  {
    if(!watchdogflag)
    {
    _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"FPS watchdog reset");
    watchdogflag = 1;
    }
    else
    {
 ...
 if(in_d[i+i+4] & 0x40)
 {
    if(!overcurrentflag)
    {
    _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"FPS over current");
    overcurrentflag = 1;
    }
    else
    {
 ...
 if( in_d[i+5] >= 0x01)   //1st_of_lp
  {
      // --------------fps_state --------------------------------//
     _f_strcpy(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr],"fps_state");

    if( (in_d[i+6] != 7) ||(in_d[i+7] != 0) ||(in_d[i+8] != 4) )  // in_d[i+6+7] packet length, in_d[i+8] is fixed 4
    {
     _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Resp Logical Pkt error");
     FPS_resp_msg_cntr++;
     FPS_resp_code = 11;
     return ;
    }
    else
    {
     ...
     switch(temp)
     {
     case(0x42): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"280 Limit Hit");
                 break;
     case(0x43): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"-10 Limit Hit");
                 break;
     }
     ...
     switch(temp)
     {
     case(0x00): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Feed Calibrated and Idle");
                 break;
     case(0x40): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Feed uncalibrated");
                 break;
     case(0x08): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Busy - Preset Run");
                 break;
     case(0x10): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Busy - Free Run");
                 break;
     case(0x4a): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Busy - Passworded Run");
                 break;
     case(0xc0): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Busy - In Calibration");
                 break;

     case(0x28): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Jammed - Preset Run");
                 break;
     case(0x30): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Jammed - Free Run");
                 break;
     case(0xe0): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Jammed - In Calibration");
                 break;
     case(0x62): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Jammed - Passworded");
                 break;
     case(0x20): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Jammed ");
                 break;
     case(0xff): _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"Ua0 OFFSET Processing");
                 break;
     } //case ends
        FPS_resp_msg_cntr++;
        //------------ fps_state end here -----------------------//
     ...
     sprintf(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr], "current_encoder");
     sprintf(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr++], "%d", EncCount);

//calculate Rpm
     Rpm = (in_d[i+12]*5*60)/2048*300;
     sprintf(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr], "current_rpm");
     sprintf(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr++], "%ld", Rpm);
    }//else
 ...
 if(in_d[i+5] == 0x02)   //2nd_of_lp = main cmd, sub cmd,
  {
    // ---------   fpscmd ---------------------------//
   sprintf(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr], "fpscmd");

   if((in_d[i+15] >6)||(in_d[i+15] < 0))
    {
    _f_strcpy(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"unknown cmd");
    FPS_resp_msg_cntr++;
    FPS_resp_code = 11;
    return;
    }
...
// Env 5vdc     in_d[i+20]
   ...
   if(in_d[i+15] == 1)  // main cmd = SET
    {
       sprintf(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr], "SET");
    }

   if(in_d[i+15] == 2)  // main cmd = READ
    {
       sprintf(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr], "READ");
    }
   ...
   if ( in_d[i+15] == 1 || in_d[i+15] == 2 )
   {
     switch(in_d[i+16])
     {
          ...
          _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_TNG_PNT");
          sprintf(par_value[TNG_PT],"%d",temp);
          // sprintf(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr++], "Set Turning Point, target: %d", temp);
          break;

          case 1:
          _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_RMP_DNCNT");
          ...
          case 2:
          _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_LRPM_LMT");
          ...
          case 3:
          _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_BRKCNT_DIFF");
          ...
          case 4:
          _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_RMP_UPCNT");
         ...
         // sprintf(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr++], "Set Ramp up Count, %d ", in_d[i+17]*5);
          ...
          case 5:
          _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_STOP_CNT");
          ...
          case 6:
          _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_MAX_PWM");
          ...
          case 7:
          _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_MAX_ANG");
          ...
          case 8:
          _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_MIN_ANG");
          temp = in_d[i+17]+(in_d[i+18]<<8);
         ...
         // sprintf(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr++], "Set Min Angle, %d ", temp*2);
          ...
          case 9:
           _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_VERSION");
           sprintf(par_value[FPS_VER],"%f",(float) in_d[i+17]/10);

          case 10 :
           _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_UA0_ANG");
           temp = 2*(in_d[i+17]+(in_d[i+18]<<8));
   ...
   {
    sprintf(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr], "Run");
    switch(in_d[i+16])
    {
          case 0: _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_CALIBRATE");break;
         // _f_strcpy(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr++],"Run to calibrate"); break;
          case 1: _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_FREE");break;
         // _f_strcpy(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr++],"Run Free"); break;
          case 2: _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_TO_RESET");break;
         // _f_strcpy(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr++],"Run to Reset"); break;
          case 3: _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_TO_FINETUNE");  break;
         // _f_strcpy(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr++],"Run to Fine Tune"); break;
          case 4: _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_PASSWORD");  break;
         // _f_strcpy(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr++],"Run Passworded"); break;
          case 5: _f_strcat( FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"_TO_UA0");  break;
         // _f_strcpy(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr++],"Run to Ua0"); break;
      }
   } else if(in_d[i+15] == 5)  // main cmd = reboot
   {
     sprintf(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr], "Reboot");
   } else if(in_d[i+15] == 6)  // main cmd = stop
   {
    // if in_d[i+15] == 5 ie reboot cmd ends
      sprintf(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr], "Stop");
   ...
   for (i = 0;i < TVAR; i++)
   {
      sprintf(FPS_MCM_Resp.resp_data.prmname[FPS_resp_msg_cntr],"%s",par_name[i]);
      sprintf(FPS_MCM_Resp.resp_data.prmvalue[FPS_resp_msg_cntr],"%s",par_value[i]);
      FPS_resp_msg_cntr++;
   }
