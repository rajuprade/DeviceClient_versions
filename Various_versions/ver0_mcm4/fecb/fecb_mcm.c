...
/* mcm4settingfordomon() => Written for DOMON command,as it does not have MCM 4 setting but used to switch OFF & ON MCM5 through MCM4 Date: 26/07/2018 -RU */
     ...
     for(i=0;i<32;i++) { digWord[i]=0; } 
                      for (j=0; j<2; j++)
                       { 
                          ...
                          // NG Cycle bit D0,1
                        getbitDO(0); for (b=0;b<2;b++) { digWord[b + (j*16)] = bitD0[b]; }
                        // NG Freq bit D2,3
                        getbitDO(0); for (b=0;b<2;b++) { digWord[ 2 + b + (j*16)] = bitD0[b]; }
                         // FE SEL D4
                           digWord[4 + (j * 16) ] = 0;
                          // Walsh Pattern D5,6,7
                             getbitDO(0); for (b=0;b<3;b++) { digWord[ 5 + b + (j*16)] = bitD0[b]; }
                           // Walsh Freq D8,9
                            getbitDO(0); for (b=0;b<2;b++) { digWord[ 8 + b + (j*16)] = bitD0[b]; }
                          // Spare 10,11
...
int  mcmPow(int mcmSW)
{
    ...
    switch(mcmSW)
    {
       case 0 : // MCM OFF
               ...
               exeCmd.CmdNum = 7;
               FECB_CTRL(MCMCARD2);  sleep(1);         //set dig mask 32 bits
               break;
   ...
   switch (fecbSet.cmdCode)
   {
       ...
       case FESDOMON  :
                     ...
                     //  if(exeCmd.McmPow == 0 )
                       ...
                       break;
          case SETURFSYS :                // Set RF, upgraded RF
	                     // MCM ON
